from rest_framework import serializers
from profiles_api import models

# Serializer is a feature from the django rest framework that allow us to easily confer data input into python object and vice-versa.
# To receive the output that comes from POST or UPDATE method. Takes care of validation rules also.

class HelloSerializer(serializers.Serializer):
    """Serializer a name field for testing out APIView"""
    name = serializers.CharField(max_length=10)


class UserProfileSerializer(serializers.ModelSerializer):
    """Serializes a user profile object"""
    # Create/Use a Meta class  to configure serializer to point to a specific model in our project

    class Meta:
        model = models.UserProfile # To point to UserProfile model in models
        fields = ('id','email','name','password')
        # Fields of the model to accessible to our API
        extra_kwargs = {
            'password': {
                'write_only': True,
                'style': {'input_type': 'password'}
            }
        }

    def create(self, validated_data):
        """Create and return a new user"""
        user = models.UserProfile.objects.create_user(
            email=validated_data['email'],
            name=validated_data['name'],
            password=validated_data['password']
        )

        return user

    def update(self, instance, validated_data):
        """Handle updating user account"""
        if 'password' in validated_data:
            password = validated_data.pop('password')
            instance.set_password(password)

        return super().update(instance, validated_data)

class ProfileFeedItemSerializer(serializers.ModelSerializer):
    """Serializes profile feed item"""

    class Meta:
        model = models.ProfileFeedItem
        fields = ('id', 'user_profile', 'status_text', 'created_on')
        extra_kwargs = {'user_profile': {'read_only': True}}
